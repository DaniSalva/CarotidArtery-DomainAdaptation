...
class unet_2D_xy(object):
    ...
    def __init__(self, sess, param_set):
        ...
        self.inputI_sizew    = param_set['inputI_sizew']
        self.inputI_sizeh    = param_set['inputI_sizeh']
    ...
    def build_online_segmentor_model(self):
        # input self.inputI_sizew, self.inputI_sizeh
        self.orig_app = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.inputI_sizew, self.inputI_sizeh, self.inputI_chn], name='orig_app')
        self.orig_label = tf.placeholder(dtype=tf.int32, shape=[self.batch_size, self.inputI_sizew, self.inputI_sizeh], name='orig_label')

        self.desired_app = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.inputI_sizew, self.inputI_sizeh, self.inputI_chn], name='desired_app')
        self.desired_strct = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.inputI_sizew, self.inputI_sizeh, self.inputI_chn], name='desired_strct')
        self.desired_pair_app = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.inputI_sizew, self.inputI_sizeh, self.inputI_chn], name='desired_pair_app')
        self.desired_pair_strct = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.inputI_sizew, self.inputI_sizeh, self.inputI_chn], name='desired_pair_strct')
        ...
        # === discriminator
        self.app_d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=(self.app_D_re), logits=self.app_D_re_logits))
        ...
        #self.pred_label_f = tf.reshape(tf.cast(self.pred_label, tf.float32, name='pred_f'),[self.batch_size, self.inputI_sizew, self.inputI_sizeh, 1])

        #self.temp_forground = self.pred_label
        #self.temp_forground = tf.cast(self.pred_label, tf.float32)
        #print("foreground:",self.temp_forground)
        self.temp_forground = self.updated_strct[:, :, :, 1]


        self.updated_forground = tf.reshape(self.temp_forground,[self.batch_size, self.inputI_sizew, self.inputI_sizeh, 1])
        #self.updated_forground = tf.cast(tf.reshape(self.pred_label, [self.batch_size, self.inputI_sizew, self.inputI_sizeh, 1]),tf.float32)
    ...
    def train_online_seg(self):
        ...
        tr_img_clec, tr_label_clec, _ = load_2d_img_pairs(self.traindata_dir, '.png', rend=0)
        ...
        for epoch in np.arange(self.epoch):
            ...
            # train batch
            rend_batch_img, rend_batch_label = get_batch_pairs(tr_img_clec, tr_label_clec, self.inputI_sizew, self.inputI_sizeh, self.batch_size, chn=1)
            app_D_batch_img, app_D_batch_label = get_batch_pairs(tr_img_clec, tr_label_clec, self.inputI_sizew, self.inputI_sizeh, self.batch_size, chn=1)

            strct_D_batch_img, strct_D_batch_label = get_batch_pairs(tr_img_clec, tr_label_clec, self.inputI_sizew, self.inputI_sizeh, self.batch_size, chn=1)
            strct_D_batch_label = strct_D_batch_label.astype('float32')
            strct_D_batch_label = np.reshape(strct_D_batch_label, [self.batch_size, self.inputI_sizew, self.inputI_sizeh, 1])

            pair_D_batch_img, pair_D_batch_label = get_batch_pairs(tr_img_clec, tr_label_clec, self.inputI_sizew, self.inputI_sizeh, self.batch_size, chn=1)
            pair_D_batch_label = pair_D_batch_label.astype('float32')
            pair_D_batch_label = np.reshape(pair_D_batch_label, [self.batch_size, self.inputI_sizew, self.inputI_sizeh, 1])
    ...
    def test_images(self):
        ...
        te_img_clec, _, data_names = load_2d_img_pairs(self.testdata_dir, '.png', rend=0,nefrona=1)

        """generate a batch of paired images for training"""
        rend_batch_img = np.zeros([self.batch_size, self.inputI_sizew, self.inputI_sizeh, 1]).astype('float32')
        #rend_batch_label = np.zeros([self.batch_size, self.inputI_sizew, self.inputI_sizeh, ]).astype('int32')
        ...
        for t in np.arange(test_s, test_e):
            ...
            temp_seg = np.reshape(temp_seg, [self.inputI_sizew, self.inputI_sizeh, ])
            temp_seg = (temp_seg * 255).astype('uint8')
            save_path = img_epoch_path + "/" + data_names[t]
    ...
    def test_online_seg(self):
        ...
        # load all image files
        tr_img_clec, tr_label_clec, _ = load_2d_img_pairs(self.traindata_dir, '.png', rend=1)
        te_img_clec, te_label_clec, _ = load_2d_img_pairs(self.testdata_dir, '.png', rend=0)
    ...
    def test_ini_rend_train(self):
        ...
        for t in np.arange(test_N):
            ...
            update_img = np.reshape(update_img, [self.inputI_sizew,self.inputI_sizeh])
