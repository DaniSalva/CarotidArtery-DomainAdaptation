...
def load_2d_img_pairs(dataset_path, img_suffix, rend=0, nefrona=0):
    img_clec = []
    label_clec = []
    data_names = []
    temp_list = glob(('{}/*' + img_suffix).format(os.path.join(dataset_path, 'us')))
    k=0
    for data_img in temp_list:
        base = os.path.basename(data_img)
        img_name = os.path.splitext(base)[0]
        if rend==0:
            img_path = os.path.join(dataset_path, 'us',img_name+img_suffix)
        else:
            img_path = os.path.join(dataset_path, 'rend_us',str(k)+img_suffix)

        data_names.append(img_name+img_suffix)
        ...
        if nefrona==0:
            lab_path = os.path.join(dataset_path, 'label', img_name+img_suffix)
            # read as grayscale
        ...
        k+=1
    return img_clec, label_clec, data_names
...
def get_batch_pairs(img_clec, label_clec, input_dimw,input_dimh, batch_size, chn=1):
    """generate a batch of paired images for training"""
    batch_img = np.zeros([batch_size, input_dimw, input_dimh, chn]).astype('float32')
    batch_label = np.zeros([batch_size, input_dimw, input_dimh]).astype('int32')
